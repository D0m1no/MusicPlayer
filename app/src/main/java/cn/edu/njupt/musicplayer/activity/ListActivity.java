package cn.edu.njupt.musicplayer.activity;
...
import cn.edu.njupt.musicplayer.R;
import cn.edu.njupt.musicplayer.adapter.AudioListAdapter;
import cn.edu.njupt.musicplayer.adapter.ViewPagerAdapter;
import cn.edu.njupt.musicplayer.entity.Audio;
import cn.edu.njupt.musicplayer.entity.AudioItem;
import cn.edu.njupt.musicplayer.service.AudioPlayService;
import cn.edu.njupt.musicplayer.util.AudioList;
import cn.edu.njupt.musicplayer.util.AudioToAudioItem;
import cn.edu.njupt.musicplayer.util.MediaUtils;
...
public class ListActivity extends AppCompatActivity implements View.OnClickListener {
    private static final String SHARED_PREFERENCES_NAME = "music_player";   // 存储路径

    private static final int PERMISSION_REQUEST_CODE = 1;
    private static String[] permissionArray = new String[] {                //权限请求数组
    ...
    private void saveStatus() {                                               //保存状态至本地
        SharedPreferences sharedPreferences = getSharedPreferences(SHARED_PREFERENCES_NAME, MODE_PRIVATE);
        SharedPreferences.Editor editor = sharedPreferences.edit();
        editor.putInt(AudioPlayService.LOOP_WAY_INT, mLoopWay);               //保存循环方式
        editor.putBoolean(AudioPlayService.LIST_SHUFFLE_BOOL, mIsShuffle);    //保存列表顺序
        editor.apply();
    }

    private void readStatus() {                                               //读取本地状态
        SharedPreferences sharedPreferences = getSharedPreferences(SHARED_PREFERENCES_NAME, MODE_PRIVATE);
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        readStatus();                                          //首先读取上次退出时的状态

        View barView = findViewById(R.id.bar);                 //实例化控件
    ...
    protected void onDestroy() {
        ...
        LocalBroadcastManager.getInstance(this).unregisterReceiver(mEventReceiver);        //取消注册服务
    }
